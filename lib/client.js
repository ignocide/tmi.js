...
class Client extends EventEmitter {
	...
	api(...args) {
		if (!_apiWarned) {
			this.log.warn(
				"Client.prototype.api is deprecated and will be removed by version 1.9.0"
			);
			_apiWarned = true;
		}
		api(...args);
			...
			switch (message.command) {
				...
				case "NOTICE": {
					...
					switch (msgid) {
						...
						case "r9k_on":
							this.log.info(
								`[${channel}] This room is now in r9k mode.`
							);
							this.emits(
						...
						case "hosts_remaining": {
							this.log.info(basicLog);
							const remainingHost = !isNaN(msg[0])
								? parseInt(msg[0])
								: 0;
							this.emits(
								["notice", "_promiseHost"],
			...
			switch (message.command) {
				case "MODE":
					if (msg === "+o") {
						...
						if (
							!this.moderators[channel].includes(
								message.params[2]
							)
						) {
					...
					if (message.tags.username === "jtv") {
						...
						if (_.hasOwn(message.tags, "bits")) {
							...
							if (_.hasOwn(message.tags, "msg-id")) {
								...
								_.hasOwn(message.tags, "custom-reward-id")
							) {
								const rewardtype =
	...
	// Send a message to channel..
	_sendReplyMessage(delay, replyId, channel, message, fn) {
		// Promise a result..
		return new Promise((resolve, reject) => {
			// Make sure the socket is opened and not logged in as a justinfan user..
			if (!this._isConnected()) {
			...
			this.ws.send(
				`@reply-parent-msg-id=${replyId} PRIVMSG ${chan} :${message}`
			);
